...
library(googledrive)
 ...
 drive_auth(email = "freun066@umn.edu")
...
```{r, echo=FALSE, warning = FALSE}
history <- if(any(most_recent_data$SepsisLabel == 1)){
most_recent_data %>% subset(most_recent_data$SepsisLabel == 1) %>% select("Patient_ID" = PatientID, "Heart_Rate" = HR, "Temperature" = Temp, "Respiratory_Rate" = Resp, "ICU_Hour"=ICULOS)} else {message("No positive patients in our dataset")}
...
most_recent_data <- most_recent_data %>% group_by(PatientID) %>% mutate(rank=dense_rank(desc(obsTime)))
